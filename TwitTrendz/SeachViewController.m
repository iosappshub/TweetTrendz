...
- (void)viewDidLoad
{
    ...
    tweetSearchBar.tintColor = [UIColor colorWithRed:248/255.0f green:248/255.0f blue:248/255.0f alpha:1.0f];
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar
{
    ...
    if ([queryString hasPrefix:@"@"]) {
        [[TwitterEngine singleton] getUserTweetsWithHandle:queryString success:^(NSArray *tweets) {
    ...
    [[TwitterEngine singleton] search:queryString success:^(NSArray *tweets) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([tweets isMemberOfClass:[NSDictionary class]]) {
                NSDictionary *errorDictionary = (NSDictionary *)tweets;
                NSArray *errorArray = [errorDictionary objectForKey:@"errors"];
